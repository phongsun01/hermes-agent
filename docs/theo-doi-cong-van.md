...
> **Trạng thái:** Đang phát triển | **Cập nhật:** 2026-06-26 ICT
...
**Config `.env`:**
```ini
CONGVAN_USER=your_username
CONGVAN_PASS=your_password
...
### F20 — Tự động kết thúc văn bản từ xa & Auto-Finish

**Trạng thái:** ✅ Đã triển khai phần Kết thúc & Auto-Finish | **Cập nhật:** 2026-06-26 ICT

Đã tích hợp tương tác Playwright để tự động thực hiện hành động xử lý văn bản "Kết thúc" từ xa qua tin nhắn và tự động quét "Vai trò" đối với văn bản mới để tự động kết thúc hàng loạt nếu "Vai trò" chỉ là "Thông báo" hoặc "Để biết".

### F20b — Tự động chuyển tiếp văn bản từ xa

**Trạng thái:** 🚧 Đang phát triển | **Cập nhật:** 2026-06-26 ICT

Hỗ trợ tương tác Playwright để chọn Chuyển từ tin nhắn Zalo hoặc tự động chuyển tiếp đến các bộ phận liên quan dựa trên quy tắc/keyword cấu hình trước.
...
### F22 — Quản lý qua Slash Command Skill (`/cc`)

**Trạng thái:** ✅ Đã triển khai | **Cập nhật:** 2026-06-26

**Mục đích:** Chuyển đổi toàn bộ thao tác thủ công (list, end, tai, tomtat, duthao) sang định dạng Hermes Skill với lệnh `/cc`.

**Kiến trúc & Bài học kinh nghiệm (Môi trường thực thi):**
- **Đăng ký lệnh**: Tạo thư mục `~/.hermes/skills/cc/` và file `SKILL.md`. Gateway tự động scan và đăng ký lệnh `/cc`. **Không cần restart gateway** — SKILL.md được đọc mỗi lần nhận lệnh, scripts `.py` chạy fresh mỗi lần. Chỉ cần `docker cp` để deploy.
- **Caveat Môi trường (WSL/Docker vs Windows Host)**:
  - Agent phục vụ Zalo Bot chạy trực tiếp trong container Linux (Docker/WSL2).
  - Các hướng dẫn trong `SKILL.md` **phải dùng đường dẫn Linux** (`/opt/data/scripts/congchuc/...`), không dùng đường dẫn Windows.
  - Không dùng `powershell` trong tool shell — dùng `python` hoặc `bash` trực tiếp.
...
```
1. Đọc metadata từ vbden_state.json (tác giả, số CV, trích yếu, trạng thái)
2. Kiểm tra attachments/<số>/
   ├─ Có file → đọc PDF (pymupdf/fitz) / DOCX (python-docx)
   └─ Chưa có → tự chạy --download-only (~25s), rồi đọc
3. Feed nội dung (max 10,000 ký tự) vào LLM
4. LLM (hermes-combo → DeepSeek-v4-flash, stream=True, max_tokens=3000)
   → Tóm tắt: Mục đích | Nội dung | Deadline | Đơn vị | Đề xuất xử lý
```
**Lưu ý kỹ thuật LLM**: `hermes-combo` route đến DeepSeek reasoning model, tiêu tốn ~1200 reasoning tokens → cần `max_tokens=3000` để còn đủ output tokens.
...
- [x] F18: Status tracking — script hoàn thành, gateway handler viết xong. Đã khắc phục lỗi lệch trạng thái giữa web và bot Zalo khi "cc end".
- [x] F22: `/cc tai <số>` gọi `--download-only` để tải ngay (không chờ cron); `/cc tomtat <số>` pipeline đầy đủ — auto-download → đọc PDF/DOCX → LLM tóm tắt (DeepSeek, max_tokens=3000).
