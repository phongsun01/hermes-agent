...
The worker automatically saves refreshed cookies every 30 minutes. This prevents session expiry during normal operation. When the worker restarts, it uses the latest saved cookies.

### Session Health Monitoring

Every 60 minutes, the worker checks if the session is still valid. If it detects authentication failures:
