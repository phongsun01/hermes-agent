...
def _run_json(cmd: list[str]) -> dict:
    try:
        ...
        raise TimeoutError("Script execution timed out") from e
    if cp.returncode != 0:
...
def _msc_token() -> str:
    val = _get_env('BOT2_MSC_TOKEN', 'MSC_TOKEN', 'MUASAMCONG_TOKEN', 'MSC_SESSION_TOKEN', default='')
    if val:
        os.environ['MSC_SESSION_TOKEN'] = val
    return val


def _msc_cookie() -> str:
    val = _get_env('BOT2_MSC_COOKIE', 'MSC_COOKIE', 'MUASAMCONG_COOKIE', default='')
    if val:
        os.environ['MSC_COOKIE'] = val
    return val
...
def _legacy_dispatch(text: str) -> MscSchema:
    ...
    if q.intent == 'lookup_pl' and q.code:
        ...
        data = _run_json(['python3', str(SCRIPTS_DIR / 'msc_pl_lookup.py'), '--pl', q.code])
    ...
    if q.intent == 'lookup_ib' and q.code:
        data = _run_json(['python3', str(SCRIPTS_DIR / 'msc_ib_lookup.py'), q.code])
        ok = data.get('status') == 'ok'
        rows = [data.get('result', data)] if ok else []
        return MscSchema(
            query_type='ib',
        ...
        if len(parts) >= 4 and parts[0] == 'v1':
            ...
            if action == 'open':
                if module == 'menu' and arg == 'root':
                    return '/menu'
                if module == 'msc' and arg in {'main', 'watch', 'watchlist', 'lookup', 'export', 'ptnt', 'hisbid', 'ls'}:
...
def _run_direct_command(raw_text: str) -> dict | None:
    ...
    if cmd == 'msc':
        ...
        if tab_in in ('ptnt', 'phantich', 'phan-tich'):
            org_code = raw_keywords.strip('"').strip("'").strip()
            data = _run_json([
            ...
            if status != 'ok':
                return {
                    ...
                    'command': 'msc',
                    'sub_action': 'ptnt',
                    'error': {'code': data.get('error', 'unknown_error'), 'message': data.get('error', 'Lỗi phân tích nhà thầu')}
                }
        ...
        if tab_in in ('ls', 'lichsu', 'lich-su'):
            try:
                sub_parts = shlex.split(raw_keywords)
            except Exception:
                sub_parts = raw_keywords.split()
            org_code = sub_parts[0].strip('"').strip("'").strip() if sub_parts else ""
            extra_args = sub_parts[1:]
            
            cmd_args = [
                'python3', str(SCRIPTS_DIR / 'msc_contractor_history.py'),
                '--org-code', org_code,
            ]
            cmd_args.extend(extra_args)
            data = _run_json(cmd_args)
            ...
            if status != 'ok':
                return {
                    ...
                    'command': 'msc',
                    'sub_action': 'ls',
                    'error': {'code': data.get('error', 'unknown_error'), 'message': data.get('error', 'Lỗi lấy lịch sử nhà thầu')}
                }
