...
def main():
    ...
    ap = argparse.ArgumentParser(description="Vietnamese public procurement bid pricing lookup")
                ...
                try:
                    for raw in p.read_text(encoding="utf-8").splitlines():
                        ...
                        if not line or line.startswith("#") or "=" not in line:
                            continue
                        k, v = line.split("=", 1)
                        if k == "MSC_COOKIE" and not cookie:
                            cookie = v.strip().strip('"').strip("'")
                except Exception:
                    pass

    if not cookie:
        print(json.dumps({"status": "error", "message": "Missing MSC Cookie. Configure in env or .env file."}, ensure_ascii=False))
        return
    ...
    raw = run_search(args.base, cookie, payload)
    try:
