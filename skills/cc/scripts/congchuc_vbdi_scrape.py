        ...
        try:
            with sync_playwright() as p:
                ...
                def _collect_for_unit(page, unit_spec):
                    if unit_spec:
                        # Go to tabid=56 to select unit (safer)
...
def main():
    ...
    # === CHECK WATCHED outgoing documents ===
    watch_file = os.path.join(STATE_DIR, "vbdi_watch.json")
    watched_items = []
    if os.path.exists(watch_file):
        try:
            with open(watch_file, "r", encoding="utf-8") as f:
                watch_data = json.load(f)
                watched_items = watch_data.get("watching", [])
        except: pass
    ...
    for item in watched_items:
        target_no = str(item.get("so", "")).strip()
        if not target_no:
            continue
        matched_doc = None
        for d in docs:  # Check against ALL documents found this run to be safe
            so_kh = str(d.get("so_ky_hieu", ""))
            trich = str(d.get("trich_yeu", ""))
            # Match if number appears in so_ky_hieu or trich_yeu
            if target_no in so_kh or target_no in trich:
                matched_doc = d
                break
        if matched_doc:
            matched_watches.append((item, matched_doc))
        else:
            remaining_watches.append(item)

    # Save remaining watches
    if watched_items:
        try:
            with open(watch_file, "w", encoding="utf-8") as f:
                json.dump({"watching": remaining_watches}, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"[ERROR] Failed to save watch file: {e}", file=sys.stderr)
    
    # Output for Zalo delivery (compact)
    # If watched docs detected, print alert first
    if matched_watches:
        print("🔔 [THEO DÕI] PHÁT HIỆN VĂN BẢN ĐI CẦN TÌM:")
        for watch_item, doc in matched_watches:
            print(f"👉 Số ký hiệu: **{doc['so_ky_hieu']}** ({doc['ngay_phat_hanh']})")
            print(f"   Trích yếu: {doc['trich_yeu']}")
            print(f"   Người soạn: {doc['nguoi_soan_thao']} | Đơn vị: {doc['don_vi_soan_thao']}")
            print("---")
        print("\n")
