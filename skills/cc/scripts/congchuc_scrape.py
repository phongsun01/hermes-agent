...
def pw_get_documents():
    ...
    def _select_unit_and_collect(page, unit_spec):
        ...
        while True:
            ...
            if not next_btn:
                break
            first_elem = page.query_selector("table.rgMasterTable > tbody > tr a")
            first_text = first_elem.inner_text().strip() if first_elem else ""
            ...
            # Wait for content to change (RadGrid postback)
            for _ in range(12):
                page.wait_for_timeout(1000)
                cur_elem = page.query_selector("table.rgMasterTable > tbody > tr a")
                cur_text = cur_elem.inner_text().strip() if cur_elem else ""
                if cur_text != first_text:
                    break
...
def main():
    ...
    # Reconcile docs:
    # 1. If a doc is currently on the web grid (current_ids), but local status was falsely set to 'done' by auto-reconciliation, restore it to 'new'
    now_ts = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    reconciled_any = False
    for sid, sdoc in state.get('documents', {}).items():
        if sid in current_ids:
            if sdoc.get('status') == 'done' and '[Tự động:' in sdoc.get('note', ''):
                sdoc['status'] = 'new'
                sdoc['note'] = re.sub(r'\[Tự động:.*?\]', '', sdoc.get('note', '')).strip()
                sdoc['status_updated_at'] = now_ts
                reconciled_any = True
        elif sdoc.get('status') == 'new' and docs:  # Only auto-done if docs were actually fetched
            sdoc['status'] = 'done'
