...
## 4. Lỗi `Playwright not available` trong Cronjob/Skill sau khi cập nhật bản mới

### Triệu chứng

Sau khi cập nhật bản mới Hermes Agent (ví dụ: checkout tag `v2026.8.3`) và build lại, các cronjob hoặc custom skills cần cào dữ liệu qua trình duyệt (như Quét công văn đi `Quet cong van di`) báo lỗi:

```
Playwright not available, cannot scrape Văn bản đi
```

### Nguyên nhân

Bản gốc của Hermes Agent trên upstream không khai báo `playwright` làm dependency chính trong `pyproject.toml`. Khi checkout sang bản mới, file `pyproject.toml` bị ghi đè làm mất đi cấu hình `"playwright==1.61.0"` mà bạn tự cấu hình trước đó, dẫn đến khi docker build nó không cài đặt thư viện này vào Python environment của container.

### Cách sửa

1. Thêm `"playwright==1.61.0"` vào phần `dependencies` của [pyproject.toml](file:///D:/Antigravity/Hermes/pyproject.toml).
2. Chạy cập nhật lockfile trên host (nếu có Python và module `uv`):
   ```powershell
   py -m uv lock
   ```
3. Khôi phục lại các file môi trường (như `.env.lightrag`) và file `docker-compose.yml` tuỳ biến của bạn từ thư mục backup gần nhất.
4. Sửa định dạng dòng cuối LF cho `docker/entrypoint.sh`:
   ```powershell
   py -c "f='D:/Antigravity/Hermes/docker/entrypoint.sh'; content = open(f, 'rb').read().replace(b'\r\n', b'\n'); open(f, 'wb').write(content)"
   ```
5. Tiến hành rebuild và khởi động lại container:
   ```powershell
   docker compose build
   docker compose up -d
   ```
...
| 4 | Lỗi `Playwright not available` sau khi update | File `pyproject.toml` bản mới ghi đè mất dependency `playwright` | Thêm lại `playwright` vào `pyproject.toml`, chạy `uv lock`, rebuild |
