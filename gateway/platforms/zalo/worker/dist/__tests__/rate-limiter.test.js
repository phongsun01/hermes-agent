...
import { parseAccessControlConfig, checkAccess, isMentioned, stripMentionPrefix, AccessDecision, } from "../access-control.js";
...
describe("checkAccess — DM", () => {
    ...
    it("allows everyone with open policy", () => {
        const result = checkAccess({ fromId: "user1", chatId: "chat1", isGroup: false, text: "" }, baseConfig);
    ...
    it("denies denylisted users", () => {
        ...
        const result = checkAccess({ fromId: "baduser", chatId: "chat1", isGroup: false, text: "" }, config);
    ...
    it("allows allowlisted users", () => {
        ...
        const result = checkAccess({ fromId: "gooduser", chatId: "chat1", isGroup: false, text: "" }, config);
        expect(result.decision).toBe(AccessDecision.ALLOW);
    });
    it("denies non-allowlisted users", () => {
        ...
        const result = checkAccess({ fromId: "otheruser", chatId: "chat1", isGroup: false, text: "" }, config);
        expect(result.decision).toBe(AccessDecision.DENY);
    });
});
describe("checkAccess — Group", () => {
    ...
    it("ignores non-mentioned messages when requireMention is true", () => {
        ...
        const result = checkAccess({ fromId: "user1", chatId: "group1", isGroup: true, text: "hello world" }, config);
    ...
    it("allows mentioned messages when requireMention is true", () => {
        ...
        const result = checkAccess({ fromId: "user1", chatId: "group1", isGroup: true, text: "@MyBot help me" }, config);
        expect(result.decision).toBe(AccessDecision.ALLOW);
    });
    it("denies all groups when policy is closed", () => {
        const config = { ...baseConfig, groupPolicy: "closed" };
        const result = checkAccess({ fromId: "user1", chatId: "group1", isGroup: true, text: "" }, config);
        expect(result.decision).toBe(AccessDecision.DENY);
    });
});
