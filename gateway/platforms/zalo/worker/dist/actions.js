import { ThreadType, Reactions, MuteAction, MuteDuration, AutoReplyScope } from "zca-js";
import { getApi, getCurrentUid } from "./client.js";
import * as nodePath from "node:path";
import { getCachedUserInfo, setCachedUserInfo, } from "./access-control.js";
...
async function safeFetch(url, maxSizeBytes = 50 * 1024 * 1024) {
    ...
    switch (p.action) {
        ...
        case "send-video": {
            if (!p.threadId || !p.url)
                throw new Error("threadId and url required");
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
            const res = await api.sendVideo({ videoUrl: p.url, thumbnailUrl: p.thumbnailUrl ?? p.url }, p.threadId, type);
        ...
        case "send-voice": {
            ...
            if (!voiceUrl)
                throw new Error("voiceUrl or url required");
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
        ...
        case "send-sticker": {
            ...
            if (p.stickerId && p.stickerCateId) {
                ...
                await api.sendSticker(detail, p.threadId, type);
                return ok({ stickerId: p.stickerId });
            }
            if (p.keyword) {
                const ids = await api.getStickers(p.keyword);
                if (!ids || ids.length === 0)
                    return ok({ error: true, message: "No stickers found" });
                const details = await api.getStickersDetail(ids[0]);
                if (!details || details.length === 0)
                    return ok({ error: true, message: "Sticker detail unavailable" });
        ...
        case "send-card": {
            if (!p.threadId || !p.userId)
                throw new Error("threadId and userId required");
        ...
        case "send-bank-card": {
            ...
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
            const res = await api.sendBankCard({ binBank: p.binBank, numAccBank: p.numAccBank, nameAccBank: p.nameAccBank }, p.threadId, type);
            return ok({ result: res });
        }
        ...
        case "send-to-stranger": {
            if (!p.userId || !p.message)
                throw new Error("userId and message required");
        ...
        case "forward-message": {
            if (!p.msgId || !p.threadIds?.length)
                throw new Error("msgId and threadIds required");
            const payload = { message: p.message || "" };
            if (p.messageTtl !== undefined)
                payload.ttl = p.messageTtl;
        ...
        case "delete-message": {
            if (!p.msgId || !p.threadId)
                throw new Error("msgId and threadId required");
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
            const cliMsgId = p.cliMsgId || p.msgId;
            const uidFrom = getCurrentUid() ?? "";
            const res = await api.deleteMessage({ data: { msgId: p.msgId, cliMsgId, uidFrom }, threadId: p.threadId, type }, Boolean(p.onlyMe));
            return ok({ result: res });
        }
        ...
        case "undo-message": {
            ...
            const res = await api.undo({ msgId: p.msgId, cliMsgId: undoCliMsgId });
            return ok({ result: res });
        }
        ...
        case "find-user": {
            ...
            if (!res?.uid)
                return ok({ found: false, phoneNumber: clean });
            return ok({
                found: true,
                user: {
                    userId: res.uid,
        ...
        case "set-friend-nickname": {
            if (!p.userId || !p.nickname)
                throw new Error("userId and nickname required");
            const uid = await resolveUserId(p.userId);
        ...
        case "get-multi-users-by-phones": {
            if (!p.phoneNumbers?.length)
                throw new Error("phoneNumbers required");
        ...
        case "create-group": {
            if (!p.groupName || !p.memberIds?.length)
                throw new Error("groupName and memberIds required");
        ...
        case "add-to-group": {
            if (!p.groupId || !p.memberIds?.length)
                throw new Error("groupId and memberIds required");
            const gid = await resolveGroupId(p.groupId);
            const ids = await Promise.all(p.memberIds.map(resolveUserId));
        ...
        case "remove-from-group": {
            if (!p.groupId || !p.userId)
                throw new Error("groupId and userId required");
            const gid = await resolveGroupId(p.groupId);
            const uid = await resolveUserId(p.userId);
        ...
        case "rename-group": {
            if (!p.groupId || !p.groupName)
                throw new Error("groupId and groupName required");
            const gid = await resolveGroupId(p.groupId);
        ...
        case "update-group-settings": {
            if (!p.groupId || !p.groupSettings)
                throw new Error("groupId and groupSettings required");
            const gid = await resolveGroupId(p.groupId);
        ...
        case "join-group-link": {
            ...
            let info = null;
            try {
                info = await api.getGroupLinkInfo(p.link);
            }
            catch { }
        ...
        case "invite-to-groups": {
            if (!p.userId || !p.groupIds?.length)
                throw new Error("userId and groupIds required");
            const uid = await resolveUserId(p.userId);
        ...
        case "change-group-avatar": {
            if (!p.groupId || !p.url)
                throw new Error("groupId and url required");
            const gid = await resolveGroupId(p.groupId);
            if (/^https?:\/\//i.test(p.url)) {
                const { buffer } = await safeFetch(p.url, 5 * 1024 * 1024);
                await api.changeGroupAvatar({ data: buffer, filename: "avatar.jpg", metadata: { totalSize: buffer.length, width: 400, height: 400 } }, gid);
            }
            else {
        ...
        case "create-poll": {
            if (!p.threadId || !p.title || !p.options?.length)
                throw new Error("threadId, title, options required");
            const pollOpts = { question: p.title, options: p.options };
            if (p.expiredTime !== undefined)
                pollOpts.expiredTime = p.expiredTime;
            if (p.allowMultiChoices !== undefined)
                pollOpts.allowMultiChoices = p.allowMultiChoices;
            if (p.allowAddNewOption !== undefined)
                pollOpts.allowAddNewOption = p.allowAddNewOption;
            if (p.hideVotePreview !== undefined)
                pollOpts.hideVotePreview = p.hideVotePreview;
            if (p.isAnonymous !== undefined)
                pollOpts.isAnonymous = p.isAnonymous;
            const res = await api.createPoll(pollOpts, p.threadId);
            return ok({ success: true, poll: res });
        }
        case "vote-poll": {
            if (!p.pollId || !p.threadId || p.optionId === undefined)
                throw new Error("pollId, threadId, optionId required");
            const res = await api.votePoll(p.pollId, p.optionId);
            return ok({ success: true, result: res });
        }
        case "lock-poll": {
            if (!p.pollId || !p.threadId)
                throw new Error("pollId and threadId required");
        ...
        case "get-poll-detail": {
            if (!p.pollId || !p.threadId)
                throw new Error("pollId and threadId required");
            const res = await api.getPollDetail(String(p.pollId));
            return ok({ poll: res });
        }
        case "add-poll-options": {
            if (!p.pollId || !p.threadId || !p.options?.length)
                throw new Error("pollId, threadId, options required");
            const res = await api.addPollOptions({ pollId: p.pollId, options: p.options.map((o) => ({ content: o, voted: false })), votedOptionIds: [] });
            return ok({ success: true, result: res });
        }
        case "share-poll": {
            if (!p.pollId || !p.threadId || !p.threadIds?.length)
                throw new Error("pollId, threadId, threadIds required");
        ...
        case "create-reminder": {
            if (!p.threadId || !p.title || !p.startTime)
                throw new Error("threadId, title, startTime required");
            const reminder = { title: p.title, startTime: p.startTime };
            if (p.emoji)
                reminder.emoji = p.emoji;
            if (p.endTime)
                reminder.endTime = p.endTime;
            if (p.repeat !== undefined)
                reminder.repeat = p.repeat;
            const res = await api.createReminder(reminder, p.threadId);
            return ok({ success: true, reminder: res });
        }
        case "edit-reminder": {
            if (!p.reminderId || !p.threadId)
                throw new Error("reminderId and threadId required");
            const reminder = {};
            if (p.title)
                reminder.title = p.title;
            if (p.startTime)
                reminder.startTime = p.startTime;
            if (p.endTime)
                reminder.endTime = p.endTime;
            if (p.emoji)
                reminder.emoji = p.emoji;
        ...
        case "set-auto-delete-chat": {
            if (!p.threadId || p.ttl === undefined)
                throw new Error("threadId and ttl required");
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
        ...
        case "update-archived-chat": {
            if (!p.threadId || p.isArchived === undefined)
                throw new Error("threadId and isArchived required");
            const type = p.isGroup ? ThreadType.Group : ThreadType.User;
        ...
        case "create-auto-reply": {
            ...
            if (p.isEnable !== undefined)
                rule.isEnable = p.isEnable;
            const res = await api.createAutoReply(rule);
            return ok({ result: res });
        }
        case "update-auto-reply": {
            if (!p.replyId)
                throw new Error("replyId required");
            const rule = {};
            if (p.title)
                rule.title = p.title;
            if (p.content)
                rule.content = p.content;
        ...
        case "update-setting": {
            if (!p.settingKey || p.settingValue === undefined)
                throw new Error("settingKey and settingValue required");
        ...
        case "change-avatar": {
            ...
            if (/^https?:\/\//i.test(p.url)) {
                const { buffer } = await safeFetch(p.url, 5 * 1024 * 1024);
                await api.changeAccountAvatar({ data: buffer, filename: "avatar.jpg", metadata: { totalSize: buffer.length, width: 400, height: 400 } });
            }
            else {
        ...
        case "reuse-avatar": {
            if (!p.photoId)
                throw new Error("photoId required");
        ...
        case "send-report": {
            if (!p.userId || !p.reason === undefined)
        ...
        case "create-note": {
            if (!p.threadId || !p.title)
                throw new Error("threadId and title required");
            const note = { title: p.title };
            if (p.description)
                note.description = p.description;
            if (p.pinAct !== undefined)
                note.pinAct = p.pinAct;
            const res = await api.createNote(note, p.threadId);
            return ok({ result: res });
        }
        case "edit-note": {
            if (!p.topicId)
                throw new Error("topicId required");
            const note = {};
            if (p.title)
                note.title = p.title;
            if (p.description)
        ...
        case "create-product": {
            if (!p.catalogId || !p.name || !p.price)
                throw new Error("catalogId, name, price required");
