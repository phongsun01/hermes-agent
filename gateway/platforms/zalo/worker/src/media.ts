 ...
 */
export function formatMarkdownToZalo(text: string): string {
    let result = text;
 ...
 */
export function truncateMessage(text: string, maxLength: number = ZALO_MAX_MESSAGE_LENGTH): { text: string, truncated: boolean } {
 ...
 */
export function prepareMessage(text: string, options?: { formatMarkdown?: boolean }): { text: string, truncated: boolean } {
    let result = text;
 ...
 */
function getCachePath(url: string, ext: string = "bin"): string {
...
export function isMediaCached(url: string): boolean {
    ...
    try {
        const meta: MediaMeta = JSON.parse(fs.readFileSync(metaPath, "utf-8"));
        const age = Date.now() - meta.cachedAt;
        if (age > MEDIA_CACHE_TTL) {
            // Expired, clean up
            try { fs.unlinkSync(getCachePath(url)); } catch {}
 ...
 */
export function getCachedMediaPath(url: string, ext: string = "bin"): string | null {
    if (!isMediaCached(url)) return null;
 ...
 */
export async function downloadAndCacheMedia(url: string, ext: string = "bin"): Promise<string> {
    // Check cache first
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (age > MEDIA_CACHE_TTL) {
                ...
                try { fs.unlinkSync(path.join(MEDIA_CACHE_DIR, baseName)); } catch {}
...
export function clearCache(): void {
    ...
    const files = fs.readdirSync(MEDIA_CACHE_DIR);
    for (const file of files) {
        try { fs.unlinkSync(path.join(MEDIA_CACHE_DIR, file)); } catch {}
    }
}
 ...
 */
export async function downloadToTemp(url: string, prefix: string = "zalo"): Promise<{ path: string, cleanup: () => void }> {
 ...
 */
export async function resolveMediaSource(source: string, prefix: string = "zalo"): Promise<{ path: string, cleanup: () => void }> {
 ...
 */
export function detectReceivedMedia(raw: any): { type: string; url?: string; mimeType?: string; fileName?: string } | null {
